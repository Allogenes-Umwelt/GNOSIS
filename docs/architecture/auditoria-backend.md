...
| Calidad de código · mypy | strict limpio | ⚠️ | **2026-09-09:** hay CHECKER en CI (`mypy.ini`) sobre una lista de módulos exentos que **solo mengua**; hoy limpio en 71 archivos. No es `--strict` a propósito: empezar por ahí sobre 16k líneas sin tipos produce una compuerta que se apaga en quince días |
...
| Dependencias · lockfile | sí | ⚠️ | **2026-09-09:** `requirements.txt` es un conjunto resuelto y CI lo usa como archivo de RESTRICCIONES, así que la suite prueba las versiones que instala la imagen. Sigue **sin hashes**, que es lo que un lockfile de verdad añade |
| Dependencias · pip-audit limpio | sí | ✅ | **2026-09-09:** limpio y **sin ignoradas** — PyPDF2 subió a 1.27.9 (última 1.x, conserva `PdfFileReader`, no toca el pipelegado) y cierra PYSEC-2022-194 y PYSEC-2026-1837. `pip-audit -r requirements.txt --strict` es compuerta HARD en CI |
